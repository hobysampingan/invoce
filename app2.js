...
async function generatePDF() {
    ...
    try {
        ...
        // Colors section dengan spacing yang lebih baik
        pdf.setFont('helvetica', 'bold');
        ...
        data.colors.forEach((color, index) => {
            ...
            pdf.setFont('helvetica', 'normal');
            pdf.text(`${color.qty} pcs`, 70, yPosition); 
            yPosition += 6; // Spacing sebelum garis
            
            // Tambahkan separator garis halus
            if (index < data.colors.length - 1) {
                pdf.setLineWidth(0.1); // Garis halus
                pdf.line(20, yPosition, 190, yPosition);
                pdf.setLineWidth(0.2); // Reset ketebalan
                yPosition += 6; // Spacing setelah garis
            } else {
                yPosition += 4; // Spacing untuk item terakhir
            }
        });
        
        yPosition += 5; // Tambah sedikit space setelah daftar warna
        ...
        // Generate Document ID dengan format DOC-YYYYMMDD-XXX
        ...
        const docId = `FNA-${dateString}-${randomLetters}${randomNum}`;

        // pdf.text(`Generated on ${new Date().toLocaleDateString('id-ID')}`, 105, yPosition, { align: 'center' });
        yPosition += 8;
        pdf.text(`Document ID: ${docId}`, 105, yPosition, { align: 'center' });
